...
class StatusConfig:
    ...
    # Лимиты по длине
    MAX_STATUS_LENGTH = 120  # Максимальная длина статуса
    MIN_STATUS_LENGTH = 1  # Минимальная длина статуса

    # Ограничения по времени
    COOLDOWN_HOURS = 1  # Часы между сменой статуса
    RESET_COOLDOWN = 24  # Часы до сброса кулдауна
    ...
    # Разрешенные символы (можно расширить)
    ALLOWED_CHARS = set(
    ...
    ADMIN_IDS = [6090751674, 987654321]  # Пример ID администраторов


# =============================================================================
# УТИЛИТЫ ДЛЯ РАБОТЫ С БАЗОЙ ДАННЫХ И ПРОВЕРКИ ПРАВ
# =============================================================================
...
class AdminChecker:
    ...
    def check_admin_permission(user_id: int) -> tuple[bool, Optional[str]]:
        """Проверяет права администратора и возвращает результат"""
...
class StatusDatabaseManager:
    ...
    def validate_status_text(text: str) -> tuple[bool, Optional[str]]:
        ...
        # Проверка символов (необязательно, можно закомментировать)
        for char in text:
    ...
    def can_change_status(last_change: Optional[datetime]) -> tuple[bool, Optional[timedelta]]:
        ...
        # Исправление: приводим все к UTC для сравнения
        now_utc = datetime.now(timezone.utc)

        # Если last_change наивное (без часового пояса), добавляем UTC
    ...
    def normalize_datetime(dt: Optional[datetime]) -> Optional[datetime]:
        ...
        if dt.tzinfo is None:
            # Наивное datetime - считаем его локальным и конвертируем в UTC
            return dt.replace(tzinfo=timezone.utc)
        else:
            # Уже с часовым поясом - конвертируем в UTC
...
class StatusService:
    ...
    def get_user_status(self, user_id: int) -> Dict[str, Any]:
        ...
        with self._db_manager.db_session() as db:
            ...
            # Нормализуем время если оно есть
    ...
    def set_user_status(self, user_id: int, status_text: str, requester_id: Optional[int] = None) -> tuple[bool, str]:
        ...
        Args:
            user_id: ID пользователя, которому устанавливается статус
            status_text: Текст статуса
            requester_id: ID пользователя, который запрашивает установку статуса
                         (None = сам пользователь устанавливает себе)
        """
        # Проверяем права, если это не сам пользователь себе устанавливает
        ...
        # Если пользователь сам себе устанавливает статус, проверяем права
        ...
        with self._db_manager.db_session() as db:
            try:
                ...
                # Проверка кулдауна (только если пользователь сам себе устанавливает)
                ...
                # Сохранение статуса с UTC временем
    ...
    def clear_user_status(self, user_id: int, requester_id: Optional[int] = None) -> tuple[bool, str]:
        ...
        Args:
            user_id: ID пользователя, которому очищается статус
            requester_id: ID пользователя, который запрашивает очистку
        """
        # Проверяем права, если это не сам пользователь себе очищает
        if requester_id is not None and requester_id != user_id:
            if not AdminChecker.is_admin(requester_id):
                return False, self._config.ERROR_MESSAGES['admin_only']

        # Если пользователь сам себе очищает статус, проверяем права
        if requester_id is None and not AdminChecker.is_admin(user_id):
            return False, self._config.ERROR_MESSAGES['no_permission']
    ...
    def format_status_display(self, status_info: Dict[str, Any]) -> str:
        ...
        changed_at = status_info['status_changed_at']

        if changed_at:
            # Приводим оба времени к UTC для корректного сравнения
            now_utc = datetime.now(timezone.utc)

            # Если changed_at уже нормализован, используем как есть
    ...
    def get_cooldown_info(self, user_id: int) -> str:
        """Получение информации о кулдауне"""
        with self._db_manager.db_session() as db:
            user = UserRepository.get_user_by_telegram_id(db, user_id)
            if not user:
    ...
    def admin_set_status(self, admin_id: int, target_user_id: int, status_text: str) -> tuple[bool, str]:
        """Администратор устанавливает статус пользователю"""
    ...
    def admin_clear_status(self, admin_id: int, target_user_id: int) -> tuple[bool, str]:
        """Администратор очищает статус пользователю"""
        if not AdminChecker.is_admin(admin_id):
            return False, self._config.ERROR_MESSAGES['no_permission']
...
class StatusHandlers:
    ...
    # ---------- КОМАНДЫ СТАТУСА (ДЛЯ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ) ----------
    ...
    async def status_command(self, message: types.Message):
        ...
        try:
            ...
            # Добавляем информацию о кулдауне (только для администраторов)
    ...
    # ---------- КОМАНДЫ СТАТУСА (ТОЛЬКО ДЛЯ АДМИНИСТРАТОРОВ) ----------
    ...
    async def status_set_command(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            # Проверяем кулдаун (только если администратор сам себе устанавливает)
            status_info = self._service.get_user_status(message.from_user.id)
            if 'error' not in status_info and status_info['status_changed_at']:
                can_change, time_left = StatusDatabaseManager.can_change_status(
                    status_info['status_changed_at']
                )
                ...
                if not can_change:
                    if time_left:
                        ...
                        minutes = (time_left.seconds % 3600) // 60
                        await message.reply(
                            f"⏳ Вы недавно меняли статус. Попробуйте через {hours}ч {minutes}м."
                        )
                    else:
    ...
    async def put_status_command(self, message: types.Message):
        ...
        try:
            ...
            # Извлекаем текст статуса из команды
            text = message.text.strip()

            # Проверяем формат команды
            if text.lower() == 'поставить статус':
                # Если только команда без текста, предлагаем ввести
                await message.reply(
            ...
            # Ищем текст после "поставить статус"
            if text.lower().startswith('поставить статус '):
                status_text = text[18:].strip()  # Убираем "поставить статус "
            elif text.lower().startswith('/поставить_статус '):
                status_text = text[18:].strip()  # Убираем "/поставить_статус "
            else:
                await message.reply("❌ Неверный формат команды. Используйте: <code>поставить статус ваш текст</code>",
                                    parse_mode=types.ParseMode.HTML)
                return
    ...
    def get_formatted_status_for_profile(self, user_id: int) -> str:
        """Получить отформатированный статус для профиля"""
        status_info = self._service.get_user_status(user_id)

        if 'error' in status_info or not status_info['has_status']:
            return ""  # Пустая строка, если нет статуса

        # Обрезаем слишком длинные статусы
        status_text = status_info['status_text']
    ...
    def get_detailed_status_for_profile(self, user_id: int) -> str:
        """Получить детальную информацию о статусе для профиля"""
        status_info = self._service.get_user_status(user_id)
...
def register_status_handlers(dp: Dispatcher) -> None:
    """Регистрация обработчиков статусов в диспетчере"""
    handlers = StatusHandlers()

    # Регистрация команд (доступны всем)
    ...
    dp.register_message_handler(
        ...
        commands=['поставить_статус', 'поставитьстатус']
    )

    dp.register_message_handler(
        handlers.status_edit_command,
        commands=['status_edit'],
        state=None
    )

    dp.register_message_handler(
        handlers.status_clear_command,
        commands=['status_clear']
    )

    dp.register_message_handler(
        handlers.status_cooldown_command,
        commands=['status_cooldown']
    )

    # Административные команды
    dp.register_message_handler(
        handlers.admin_set_status_command,
        commands=['admin_set_status'],
        state=None
    )

    dp.register_message_handler(
        handlers.admin_clear_status_command,
        commands=['admin_clear_status']
    )

    # Регистрация текстовых команд (только для администраторов)
    dp.register_message_handler(
        handlers.status_command,
